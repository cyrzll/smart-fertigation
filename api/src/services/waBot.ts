...
Status        : Aktif`;
          ...
          // Menu 6: MODE DEMO & MANUAL CONTROL
            ...
            const controlDevice = userDevices.find((d: any) => isDeviceSocketConnected(d.device_code, d.serial_code)) || primaryDevice;
            ...
            const valveList = manualValves.length > 0
              ? manualValves.map((v: any, index: number) =>
                  `${index + 1}. *${v.name}*\n   GPIO ${v.gpio || '-'}${v.description ? ` — ${v.description}` : ''}`
                ).join('\n')
          ...
          // Handle ON/OFF Test Valve Manual Commands (e.g. ON 1 10, OFF 1)
            ...
            if (!valve) {
              ...
              const [insRes]: any = await pool.query(
                ...
                [userId, controlDevice.id, valve.id, action === 'ON' ? 'OPEN' : 'CLOSE', durationSec, 'running', nowStr]
              );

              const cmdId = insRes.insertId;
              const sent = sendValveCommandToDevice(
                controlDevice.device_code,
                controlDevice.serial_code,
                cmdId,
                Number(valve.gpio),
              ...
              if (!sent) {
                await pool.query(
                  ...
                  ['WebSocket perangkat tidak tersedia.', cmdId]
                );
