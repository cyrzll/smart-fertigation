...
export async function initDb() {
  try {
    ...
    try { await conn.query('ALTER TABLE users ADD COLUMN uid VARCHAR(50) UNIQUE NULL AFTER id'); } catch (_) {}
    try { await conn.query('ALTER TABLE users ADD COLUMN username VARCHAR(255) UNIQUE NULL AFTER name'); } catch (_) {}
    try { await conn.query('ALTER TABLE users ADD COLUMN phone VARCHAR(50) NULL AFTER email'); } catch (_) {}
    ...
    try { await conn.query('DROP TABLE IF EXISTS user_wa_numbers'); } catch (_) {}
    ...
    try { await conn.query('ALTER TABLE fertigation_schedules MODIFY COLUMN hst INT NULL DEFAULT 0'); } catch (_) {}
    try { await conn.query('ALTER TABLE fertigation_schedules ADD COLUMN hst_start INT UNSIGNED NOT NULL DEFAULT 0'); } catch (_) {}
    ...
    try { await conn.query('ALTER TABLE fertigation_profiles ADD COLUMN user_id INT NULL AFTER id'); } catch (_) {}
    ...
    if (sensorCount[0].count === 0) {
      await conn.query(
        ...
        ['ESP-FERTIGASI-01', null, null, null, null, 1.8, 900.0, 'Normal']
      );
    }
    ...
    if (usersCount[0].count === 0) {
      await conn.query(
        'INSERT INTO users (uid, name, username, email, phone, password, level) VALUES (?, ?, ?, ?, ?, ?, ?)',
        ['USR-0001', 'Administrator', 'admin', 'admin@smart.com', '081234567890', 'admin123', 'admin']
      );
      await conn.query(
        'INSERT INTO users (uid, name, username, email, phone, password, level) VALUES (?, ?, ?, ?, ?, ?, ?)',
        ['USR-0002', 'Operator Kebun', 'operator', 'user@smart.com', '089876543210', 'user123', 'user']
      );
    }

    // Seed default growth phases if empty
    const [phasesCount]: any = await conn.query('SELECT COUNT(*) as count FROM growth_phases');
    ...
    const [profiles]: any = await conn.query('SELECT COUNT(*) as count FROM fertigation_profiles');
    if (profiles[0].count === 0) {
      const [resProfile]: any = await conn.query(
        'INSERT INTO fertigation_profiles (name, description, is_active) VALUES (?, ?, 1)',
        ['Melon Standar', 'Profil fertigasi standar tanaman melon berdasarkan fase pertumbuhan dan HST.']
      );
      ...
      await conn.query(
        'INSERT INTO fertigation_schedules (fertigation_profile_id, valve_id, growth_phase_id, hst_start, hst_end, start_time, duration_seconds) VALUES (?, ?, 2, 1, 30, ?, ?)',
        [profileId, v1, '06:00:00', 180]
      );
      await conn.query(
        'INSERT INTO fertigation_schedules (fertigation_profile_id, valve_id, growth_phase_id, hst_start, hst_end, start_time, duration_seconds) VALUES (?, ?, 2, 1, 30, ?, ?)',
        [profileId, v1, '12:00:00', 180]
      );
      await conn.query(
        'INSERT INTO fertigation_schedules (fertigation_profile_id, valve_id, growth_phase_id, hst_start, hst_end, start_time, duration_seconds) VALUES (?, ?, 2, 1, 30, ?, ?)',
        [profileId, v1, '16:00:00', 180]
      );

      // Seed default active planting
      const [plantingsCount]: any = await conn.query('SELECT COUNT(*) as count FROM plantings');
      if (plantingsCount[0].count === 0) {
        await conn.query(
          'INSERT INTO plantings (name, planting_date, fertigation_profile_id, is_active) VALUES (?, ?, ?, 1)',
          ['Melon Greenhouse A', '2026-08-10', profileId]
        );
      }

      // Seed default ESP32 device
      const [devicesCount]: any = await conn.query('SELECT COUNT(*) as count FROM devices');
      if (devicesCount[0].count === 0) {
        await conn.query(
          'INSERT INTO devices (name, device_code, mode, current_hst, firmware_version) VALUES (?, ?, ?, ?, ?)',
          ['ESP32 Fertigasi Utama', 'ESP-FERTIGASI-01', 'AUTO', 11, 'v2.0.0-Baileys']
        );
      }
    }
