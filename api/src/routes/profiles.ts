...
app.get('/', async (c) => {
  try {
    const [profiles]: any = await pool.query('SELECT * FROM fertigation_profiles ORDER BY name ASC');
...
app.put('/:id', async (c) => {
  try {
    ...
    await pool.query(
      'UPDATE fertigation_profiles SET name = ?, description = ? WHERE id = ?',
...
app.patch('/:id/toggle', async (c) => {
  try {
    const id = c.req.param('id');
    await pool.query(
      'UPDATE fertigation_profiles SET is_active = NOT is_active WHERE id = ?',
      [id]
    );
...
app.delete('/:id', async (c) => {
  try {
    ...
    const [schedules]: any = await pool.query(
      'SELECT COUNT(*) as count FROM fertigation_schedules WHERE fertigation_profile_id = ?',
      [id]
    );
    ...
    const [plantings]: any = await pool.query(
      'SELECT COUNT(*) as count FROM plantings WHERE fertigation_profile_id = ?',
      [id]
    );
