...
app.post('/device/heartbeat', async (c) => {
    try {
        ...
        const [devices] = await pool.query('SELECT * FROM devices WHERE device_code = ? OR (serial_code = ? AND serial_code IS NOT NULL) LIMIT 1', [device_code || '', serial_code || '']);
        const nowStr = new Date().toISOString().replace('T', ' ').slice(0, 19);
        if (devices.length === 0) {
...
app.post('/device/commands/claim', async (c) => {
    try {
        ...
        const [devices] = await pool.query('SELECT * FROM devices WHERE device_code = ?', [device_code]);
        if (devices.length === 0) {
...
app.post('/device/commands/:id/complete', async (c) => {
    try {
        ...
        const device = devices[0];
        const [cmds] = await pool.query('SELECT * FROM device_commands WHERE id = ?', [id]);
...
app.get('/device/telemetry', async (c) => {
    try {
        const [rows] = await pool.query('SELECT * FROM sensor_telemetry ORDER BY created_at DESC LIMIT 1');
        if (rows.length === 0) {
            return c.json({
                success: true,
                data: { suhu: null, kelembaban: null, media: null, level_air: null, ec: null, tds: null, status: 'Belum ada data' }
            });
        }
...
app.post('/device/telemetry', async (c) => {
    try {
        const body = await c.req.json();
        const { device_code, suhu, suhu_air, kelembaban, media, level_air, ec, tds, status } = body;
        ...
        await pool.query('INSERT INTO sensor_telemetry (device_code, suhu, suhu_air, kelembaban, media, level_air, ec, tds, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [device_code || 'ESP-FERTIGASI-01', suhu ?? null, suhu_air ?? null, kelembaban ?? null, media ?? null, level_air ?? null, finalEc, finalTds, status || 'Normal']);
