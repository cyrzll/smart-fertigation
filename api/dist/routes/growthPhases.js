...
app.get('/', async (c) => {
    try {
        const [phases] = await pool.query('SELECT * FROM growth_phases ORDER BY id ASC');
...
app.post('/', async (c) => {
    try {
        ...
        const [res] = await pool.query('INSERT INTO growth_phases (name, description, is_active) VALUES (?, ?, ?)', [name.trim(), description || null, activeVal]);
...
app.put('/:id', async (c) => {
    try {
        ...
        const activeVal = is_active === undefined ? 1 : (is_active ? 1 : 0);
        await pool.query('UPDATE growth_phases SET name = ?, description = ?, is_active = ? WHERE id = ?', [name.trim(), description || null, activeVal, id]);
