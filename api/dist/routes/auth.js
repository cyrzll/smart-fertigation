...
app.post('/login', async (c) => {
    try {
        ...
        const [rows] = await pool.query('SELECT id, uid, name, username, email, phone, password, level FROM users WHERE email = ? OR username = ? LIMIT 1', [identifier, identifier]);
        if (rows.length === 0) {
        ...
        const [waNumbers] = await pool.query("SELECT id, uid, whatsapp_number, otp, status, created_at FROM user_whatsapp_numbers WHERE uid = ? ORDER BY id ASC", [uid]);
        return c.json({
            success: true,
...
app.post('/register', async (c) => {
    try {
        ...
        const [existing] = await pool.query('SELECT id, email, username FROM users WHERE email = ? OR (username IS NOT NULL AND username = ?) LIMIT 1', [email, username || '']);
        ...
        const [res] = await pool.query('INSERT INTO users (name, username, email, phone, password, level) VALUES (?, ?, ?, ?, ?, ?)', [name, username || null, email, phone || null, password, 'user']);
        ...
        if (phone) {
            ...
            await pool.query("INSERT INTO user_whatsapp_numbers (uid, whatsapp_number, otp, status) VALUES (?, ?, ?, 'pending')", [uid, cleanPhone, otpCode]);
            sendWaMessage(cleanPhone, `🔐 *KODE OTP VERIFIKASI SMART FERTIGATION*\n\nKode OTP Anda adalah: *${otpCode}*\n\nMasukkan kode ini pada dashboard atau balaskan *OTP ${otpCode}* via WhatsApp ini untuk memverifikasi nomor Anda.`).catch(() => { });
        }
...
app.get('/users', async (c) => {
    try {
        const [users] = await pool.query('SELECT id, uid, name, username, email, phone, level, created_at FROM users ORDER BY id ASC');
        for (const u of users) {
            const uid = await ensureUserUid(u);
            const [numbers] = await pool.query("SELECT id, uid, whatsapp_number, otp, status, created_at FROM user_whatsapp_numbers WHERE uid = ? ORDER BY id ASC", [uid]);
            u.waNumbers = numbers;
        }
...
app.get('/users/:userId/wa-numbers', async (c) => {
    try {
        ...
        const [uRows] = await pool.query('SELECT id, uid FROM users WHERE id = ? LIMIT 1', [userId]);
        if (uRows.length === 0) {
...
app.post('/users/:userId/wa-numbers', async (c) => {
    try {
        ...
        const [existingCount] = await pool.query('SELECT COUNT(*) as count FROM user_whatsapp_numbers WHERE uid = ?', [uid]);
        ...
        const [dup] = await pool.query('SELECT id FROM user_whatsapp_numbers WHERE uid = ? AND whatsapp_number = ? LIMIT 1', [uid, cleanPhone]);
        if (dup.length > 0) {
            return c.json({ success: false, message: 'Nomor WhatsApp ini sudah terdaftar pada akun Anda.' }, 400);
        }
        const otpCode = String(Math.floor(100000 + Math.random() * 900000));
        const [res] = await pool.query("INSERT INTO user_whatsapp_numbers (uid, whatsapp_number, otp, status) VALUES (?, ?, ?, 'pending')", [uid, cleanPhone, otpCode]);
        let sentWa = false;
        try {
            sentWa = await sendWaMessage(cleanPhone, `🔐 *KODE OTP VERIFIKASI SMART FERTIGATION*\n\nKode OTP Anda adalah: *${otpCode}*\n\nMasukkan kode ini pada dashboard atau balaskan *OTP ${otpCode}* via WhatsApp ini untuk memverifikasi nomor Anda.`);
        }
        catch (_) { }
        return c.json({
            success: true,
...
app.post('/users/:userId/wa-numbers/:numberId/verify', async (c) => {
    try {
        ...
        const [rows] = await pool.query('SELECT * FROM user_whatsapp_numbers WHERE id = ? AND uid = ? LIMIT 1', [numberId, uid]);
        if (rows.length === 0) {
        ...
        await pool.query("UPDATE user_whatsapp_numbers SET status = 'verified', otp = NULL WHERE id = ?", [numberId]);
        try {
            await sendWaMessage(record.whatsapp_number, `✅ *NOMOR WHATSAPP TERVERIFIKASI!*\n\nNomor WhatsApp Anda (${record.whatsapp_number}) telah terverifikasi untuk memonitor & mengontrol sistem Smart Fertigation ESP32.`);
        }
        catch (_) { }
...
app.post('/users/:userId/wa-numbers/:numberId/resend', async (c) => {
    try {
        ...
        await pool.query("UPDATE user_whatsapp_numbers SET otp = ?, status = 'pending' WHERE id = ?", [newOtp, numberId]);
        await sendWaMessage(record.whatsapp_number, `🔐 *KODE OTP BARU SMART FERTIGATION*\n\nKode OTP Anda adalah: *${newOtp}*\n\nMasukkan kode ini pada dashboard atau balaskan *OTP ${newOtp}* via WhatsApp ini.`);
...
app.put('/users/:id', async (c) => {
    try {
        ...
        if (newEmail !== current.email) {
            const [emailDup] = await pool.query('SELECT id FROM users WHERE email = ? AND id != ? LIMIT 1', [newEmail, id]);
        ...
        if (newUsername && newUsername !== current.username) {
            const [userDup] = await pool.query('SELECT id FROM users WHERE username = ? AND id != ? LIMIT 1', [newUsername, id]);
            if (userDup.length > 0) {
                return c.json({ success: false, message: 'Username sudah digunakan oleh akun lain.' }, 400);
            }
        }
        await pool.query('UPDATE users SET name = ?, username = ?, email = ?, phone = ?, password = ? WHERE id = ?', [newName, newUsername, newEmail, newPhone, newPassword, id]);
        const [updatedRows] = await pool.query('SELECT id, uid, name, username, email, phone, level FROM users WHERE id = ? LIMIT 1', [id]);
...
app.get('/users/:id/devices', async (c) => {
    try {
        ...
        const [devices] = await pool.query('SELECT * FROM devices WHERE user_id = ? ORDER BY id DESC', [userId]);
...
app.post('/users/:id/devices', async (c) => {
    try {
        ...
        // Get user UID
        const [uRows] = await pool.query('SELECT uid FROM users WHERE id = ?', [userId]);
        const userUid = uRows[0]?.uid || `USR-${String(userId).padStart(4, '0')}`;
        // Check if device_code already exists
        const [existing] = await pool.query('SELECT * FROM devices WHERE device_code = ?', [trimmedCode]);
        if (existing.length > 0) {
            ...
            await pool.query('UPDATE devices SET user_id = ?, uid = ?, name = ?, mode = COALESCE(?, mode), firmware_version = COALESCE(?, firmware_version), is_active = 1 WHERE id = ?', [userId, userUid, name.trim(), mode || null, firmware_version || null, dev.id]);
            return c.json({ success: true, message: 'Perangkat ESP32 berhasil dihubungkan ke akun Anda.' });
        }
        await pool.query('INSERT INTO devices (user_id, uid, name, device_code, mode, firmware_version, status, is_active) VALUES (?, ?, ?, ?, ?, ?, "verified", 1)', [userId, userUid, name.trim(), trimmedCode, mode || 'AUTO', firmware_version || '1.0.0']);
...
app.post('/users/:id/devices/request-verify', async (c) => {
    try {
        ...
        const [existing] = await pool.query('SELECT * FROM devices WHERE (serial_code = ? AND serial_code IS NOT NULL AND serial_code != "") OR (device_code = ? AND device_code IS NOT NULL AND device_code != "") LIMIT 1', [trimmedSerial, trimmedCode]);
        let deviceId = null;
...
app.put('/users/:id/devices/:deviceId', async (c) => {
    try {
        ...
        const [existing] = await pool.query('SELECT * FROM devices WHERE id = ?', [deviceId]);
        if (existing.length === 0) {
            return c.json({ success: false, message: 'Perangkat tidak ditemukan.' }, 404);
        }
        if (device_code) {
            const trimmedCode = device_code.trim().toUpperCase();
            const [dup] = await pool.query('SELECT id, user_id FROM devices WHERE (device_code = ? OR serial_code = ?) AND id != ?', [trimmedCode, trimmedCode, deviceId]);
            if (dup.length > 0) {
        ...
        await pool.query('UPDATE devices SET name = COALESCE(?, name), device_code = COALESCE(?, device_code), mode = COALESCE(?, mode), is_active = COALESCE(?, is_active) WHERE id = ? AND (user_id = ? OR user_id IS NULL)', [name ? name.trim() : null, device_code ? device_code.trim().toUpperCase() : null, mode || null, is_active !== undefined ? is_active : null, deviceId, userId]);
